...
static int	path_check(t_world *world, int y, int x, int *col)
{
	...
	if (world->copy[y][x] == 'E')
		(*col)++;
	world->copy[y][x] = 'S';
	if (path_check(world, y, x +1, col) || path_check(world, y, x -1, col)
		|| path_check(world, y +1, x, col) || path_check(world, y -1, x, col))
		return (1);
...
int	valid_path(t_world *world)
{
	...
	copy_grid(world);
	if (!path_check(world, world->player.pos.y,
